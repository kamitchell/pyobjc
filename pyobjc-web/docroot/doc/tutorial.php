...
<li>Create a work directory <tt class="literal"><span class="pre">src</span></tt>. Check which Python you have installed PyObjC
...
<tt class="literal"><span class="pre">/usr/local/bin/python</span></tt> but Apple's <tt class="literal"><span class="pre">/usr/bin/python</span></tt> comes first in your
<tt class="literal"><span class="pre">$PATH</span></tt>. Make sure you use the right python whereever it says <tt class="literal"><span class="pre">python</span></tt> in this
tutorial.</li>
<li>Start Interface Builder, select <em>Cocoa Application</em>
in the new file dialog, save this file as <tt class="literal"><span class="pre">src/MainMenu.nib</span></tt>.</li>
...
<tt class="literal"><span class="pre">nibclassbuilder</span></tt> will parse the NIB file and create a skeleton module for you.
Invoke it as follows (from the <tt class="literal"><span class="pre">src</span></tt> directory):</p>
<pre class="literal-block">
$ python -c &quot;import PyObjCScripts.nibclassbuilder&quot; English.lproj/MainMenu.nib &gt; CurrencyConverter.py
</pre>
<p>Depending on your installation, the <tt class="literal"><span class="pre">nibclassbuilder</span></tt> script may be on your <tt class="literal"><span class="pre">$PATH</span></tt>.
...
create a file named <tt class="literal"><span class="pre">setup.py</span></tt> with the following contents:</p>
<pre class="literal-block">
...
<p>Note that we use the <tt class="literal"><span class="pre">-A</span></tt> argument to create an alias bundle at 
<tt class="literal"><span class="pre">dist/CurrencyConverter.app</span></tt>.  Alias bundles contain an alias to the
main script (<tt class="literal"><span class="pre">CurrencyConverter.py</span></tt>) and symlinks to the data files
(<tt class="literal"><span class="pre">English.lproj</span></tt>), rather than including them and their dependencies
...
<ul>
<li><p class="first">double-click <tt class="literal"><span class="pre">dist/CurrencyConverter</span></tt> from the Finder
...
is going on if there are errors, and it allows you to interact with <tt class="literal"><span class="pre">pdb</span></tt>
...
can be viewed with <tt class="literal"><span class="pre">/Applications/Utilities/Console.app</span></tt>.</p>
...
<li><p class="first">Time to actually write some code. Open <tt class="literal"><span class="pre">CurrencyConverter.py</span></tt> in your favorite
...
The main problem, which may be obvious, is that we cannot run <tt class="literal"><span class="pre">nibclassbuilder</span></tt> again
...
<em>NOTE:</em> you drag to the <em>instance</em> of <tt class="literal"><span class="pre">ConverterController</span></tt>, not to the class.
In the <em>Info</em> panel, <em>Connections</em> section, select <tt class="literal"><span class="pre">invertRate:</span></tt> and press <em>Connect</em>.</p>
</li>
...
<p>When running in this mode, we will get a <tt class="literal"><span class="pre">pdb.post_mortem(...)</span></tt> console in the terminal
instead of the alert panel.  You can see this in action if you try and invert an exchange
rate of <tt class="literal"><span class="pre">0</span></tt>.</p>
</li>
<li><p class="first">Fix the final bug by testing for <tt class="literal"><span class="pre">rate</span> <span class="pre">==</span> <span class="pre">0.0</span></tt> in <tt class="literal"><span class="pre">invertRate_()</span></tt>. The result is in the
<a class="reference" href="step12-src">step12-src</a> directory.</p>
</li>
</ol>
</div>
<div class="section" id="creating-a-redistributable-application">
<h1><a name="creating-a-redistributable-application">Creating a redistributable application</a></h1>
<p>Your application is finished, and you want to run it on other computers, or
simply just move it to the <tt class="literal"><span class="pre">Applications</span></tt> folder (or anywhere else) and
insulate it from the original source code.</p>
<p>This can be done with the following steps from the <tt class="literal"><span class="pre">src</span></tt> directory:</p>
<pre class="literal-block">
$ rm -rf dist
$ python setup.py py2app
</pre>
<p>Now the application bundle located at <tt class="literal"><span class="pre">dist/CurrencyConverter.app</span></tt> is a fully
